...
export function Countdown({ exactEnd, startTime, showMessage = true }: { exactEnd?: Date, startTime?: Date, showMessage?: boolean }) {
  // get end/beginning times
  const end = useMemo(() => (exactEnd ? Math.floor(exactEnd.getTime() / 1000) : 0), [
    exactEnd
  ])
  const begin = useMemo(() => (startTime ? startTime.getTime() / 1000 : 0), [startTime])
