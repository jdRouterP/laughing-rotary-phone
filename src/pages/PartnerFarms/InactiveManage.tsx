...
import { useInactiveStakingInfo } from '../../state/partnerFarms/hooks'
...
export default function Manage({
  ...
  const stakingInfo = useInactiveStakingInfo(stakingTokenPair)?.[0]
        ...
        <PoolData>
          {
            !stakingInfo?.hasClaimedPartial || !stakingInfo?.vestingActive ? <AutoColumn gap="sm">
              ...
              <AutoColumn gap="sm">
                <TYPE.body style={{ margin: 0 }}>{`Total ${stakingInfo ? stakingInfo?.rewardToken.symbol : ""} earned (includes vesting)`}</TYPE.body>
                <TYPE.body fontSize={22} fontWeight={500}>
                  {` ${stakingInfo?.hasClaimedPartial ? totalEarnedReward : stakingInfo?.earnedAmount?.toFixed(2) ?? '0'} ${stakingInfo ? stakingInfo?.rewardToken.symbol : ""}`}
                </TYPE.body>
              </AutoColumn>
          }
        ...
        <BottomSection gap="lg" justify="center">
          ...
          <StyledBottomCard dim={stakingInfo?.earnedAmount?.equalTo(JSBI.BigInt(0))}>
            ...
            <AutoColumn gap="sm">
              <RowBetween>
                <div>
                  <TYPE.black>{`Your unclaimed ${stakingInfo ? stakingInfo?.rewardToken.symbol : "DFYN"}`}</TYPE.black>
                </div>
