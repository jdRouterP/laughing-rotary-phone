...
import { CardSection, DataCard, CardNoise, CardBGImage } from '../../components/customFloraFarms/styled'
import { ButtonPrimary } from '../../components/Button'
import StakingModal from '../../components/customFloraFarms/StakingModal'
import { useInactiveStakingCustomFloraFarmInfo } from '../../state/custom-flora-farm-stake/hooks'
import UnstakingModal from '../../components/customFloraFarms/UnstakingModal'
import ClaimRewardModal from '../../components/customFloraFarms/ClaimRewardModal'
import BurnConfirmationModal from '../../components/customFloraFarms/BurnConfirmationModal'
...
export default function Manage({
  ...
  const dualContract = useBYOFFloraFarmsContract(stakingAddress)
  const lpAddress = useSingleCallResult(dualContract, 'stakingToken')
  ...
  const stakingInfo = useInactiveStakingCustomFloraFarmInfo(stakingAddress)?.[0]
  ...
  let totalEarnedRewardTwo: string | undefined = undefined
  let countUpAmount: string = ""
  let countUpAmountTwo: string | undefined = undefined
  if(stakingInfo?.rewardToken.length === 1){
    ...
    totalEarnedReward = stakingInfo?.totalEarnedRewardArr[0]?.toFixed(2) ?? '0';
    countUpAmount = stakingInfo?.earnedAmount[0]?.toFixed(6) ?? '0'
    totalEarnedRewardTwo = stakingInfo?.totalEarnedRewardArr[1]?.toFixed(2) ?? '0'
    countUpAmountTwo = stakingInfo?.earnedAmount[1]?.toFixed(6) ?? '0'
  }
    ...
    <PageWrapper gap="lg" justify="center">
        ...
        <PoolData>
          {
            !stakingInfo?.hasClaimedPartial || !stakingInfo?.vestingActive ? <AutoColumn gap="sm">
              ...
              <AutoColumn gap="sm">
                <TYPE.body style={{ margin: 0 }}>{`Total ${stakingInfo?.rewardToken[0].symbol} ${stakingInfo?.rewardToken.length === 2 ? stakingInfo?.rewardToken[1].symbol : ""} earned (includes vesting)`}</TYPE.body>
                <TYPE.body fontSize={22} fontWeight={500}>
                  {` ${stakingInfo?.hasClaimedPartial ? totalEarnedReward : stakingInfo?.earnedAmount[0]?.toFixed(2) ?? '0'} ${stakingInfo?.rewardToken[0].symbol} ${stakingInfo?.rewardToken.length === 2 ? (stakingInfo?.hasClaimedPartial ? totalEarnedRewardTwo : stakingInfo?.earnedAmount[1]?.toFixed(2)) : ""} ${stakingInfo?.rewardToken.length === 2 ? stakingInfo?.rewardToken[1].symbol : ""}`}
                </TYPE.body>
              </AutoColumn>
          }
      ...
      <PositionInfo gap="lg" justify="center" dim={showAddLiquidityButton}>
        <BottomSection gap="lg" justify="center">
          ...
          <StyledBottomCard dim={stakingInfo?.earnedAmount[0]?.equalTo(JSBI.BigInt(0))}>
            <CardBGImage desaturate />
            <CardNoise />
            <AutoColumn gap="sm">
              <RowBetween>
