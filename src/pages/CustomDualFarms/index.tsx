...
export default function CustomDualFarms() {
  const { chainId } = useActiveWeb3React()
  const {dualFarms} = useCustomFarmInfo()
  
  // staking info for connected account
  // staking info for connected account
  const stakingInfos = useStakingDualFarmInfo();
  /**
  ...
  const stakingRewardsExist = Boolean(typeof chainId === 'number' && (dualFarms.length ?? 0) > 0)
          ...
          <DataCard>
            ...
            <CardSection>
              <AutoColumn gap="md">
                <TopSectionHeader>
                  <RowBetween>
                    <TYPE.white fontWeight={600}>Custom Dual Farms</TYPE.white>
                  </RowBetween>
                  <StyledInternalLink to={`/custom-dual-farms/archived`} style={{ width: '100%', color: '#ff007a' }}>
                ...
                <RowBetween>
                  <TYPE.white fontSize={14}>
                    These Farms Contains all farms build by build your own farms
                  </TYPE.white>
                </RowBetween>{' '}
                <ExternalLink
