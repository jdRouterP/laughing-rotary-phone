...
import { Countdown } from 'pages/CustomPopularFarms/Countdown'
...
export default function PoolCardCustomFloraFarm({ stakingInfo, isInactive }: { stakingInfo: StakingInfo, isInactive: boolean }) {
  ...
  return (
    <Wrapper showBackground={isStaking} bgColor={backgroundColor}>
      ...
      <TopSection>
        ...
        <StyledInternalLink to={`/custom-popular-farms/${isInactive ? 'archived/' : ''}${stakingInfo?.stakingRewardAddress}`} style={{ width: '100%' }}>
      ...
      <StatContainer>
        ...
        {stakingInfo?.active &&
          ...
          //           ?.toFixed(0, { groupSeparator: ',' })} ${stakingInfo ? stakingInfo?.rewardToken.symbol : "DFYN"} / day`
          //         : '0 DFYN / day'
          //       : '-'}
          //   </TYPE.white>
          // </RowBetween>
                ...
                {stakingInfo
                  ? stakingInfo.active
                    ? `${Number(stakingInfo.totalRewardRate[0]?.toExact()) === 0 
                      ...
                      : stakingInfo.totalRewardRate[0]?.multiply(BIG_INT_SECONDS_IN_DAY)
                  ...
                  <TYPE.white>
                    {stakingInfo
                      ? stakingInfo.active
                        ...
                        : `{${Math.round((Number(stakingInfo.rewardAmount[1])/Math.pow(10, stakingInfo.rewardToken[1]?.decimals))/Number(stakingInfo.duration))} ${stakingInfo && stakingInfo.rewardToken[1]?.symbol} / day}`
                      : '-'}
                  </TYPE.white>
              </RowBetween>
            }
              ...
              {stakingInfo
                ? stakingInfo.active
                  ? `${stakingInfo.rewardRate
                    ?.multiply(BIG_INT_SECONDS_IN_DAY)
                    ?.toSignificant(4, { groupSeparator: ',' })} ${stakingInfo ? stakingInfo?.rewardToken.symbol : "DFYN"} / day`
                  : '0 DFYN / day'
                : '-'} */}
                <span role="img" aria-label="wizard-icon" style={{ marginRight: '0.5rem' }}>
                ⚡
