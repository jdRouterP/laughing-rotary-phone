...
import { StakingInfo } from '../../state/custom-vanilla-stake/hooks'
...
import { Countdown } from 'pages/CustomVanillaFarms/Countdown'
...
export default function PoolCardCustomVanilla({ stakingInfo, isInactive }: { stakingInfo: StakingInfo, isInactive: boolean }) {
  ...
  return (
    <Wrapper showBackground={isStaking} bgColor={backgroundColor}>
      ...
      <TopSection>
        ...
        <StyledInternalLink to={`/custom-eco-farms/${isInactive ? 'archived/' : ''}${stakingInfo.stakingRewardAddress}`} style={{ width: '100%' }}>
      ...
      <StatContainer>
        ...
        <RowBetween>
          ...
          <TYPE.white>{`${Number(valueOfTotalStakedAmountInUSDC?.toSignificant(6)) === 0 ? '-' : apr ? apr?.toFixed(2) : '-'}%`}</TYPE.white>
        </RowBetween>
      </StatContainer>
              ...
              {stakingInfo
                ? stakingInfo.active
                  ? `${stakingInfo.rewardRate
                    ?.multiply(BIG_INT_SECONDS_IN_DAY)
                    ?.toSignificant(4, { groupSeparator: ',' })} ${stakingInfo?.rewardToken?.symbol ?? 'DFYN'} / day`
                  : '0 DFYN / day'
                : '-'}
            </TYPE.black>
          </BottomSection>
