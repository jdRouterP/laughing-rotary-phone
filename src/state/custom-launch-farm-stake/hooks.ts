...
import { BYOF_LAUNCH_FARM_INTERFACE } from 'constants/abis/launch-farm-byof'
...
export function useStakingCustomLaunchFarmInfo(pairToFilterBy?: string): StakingInfo[] {
  ...
  const info = useMemo(
    () =>
      chainId
        ? launchFarms?.filter(stakingRewardInfo =>
        ...
        : [],
    [chainId, pairToFilterBy, launchFarms]
  )
  ...
  const balances = useMultipleContractSingleData(rewardsAddresses, BYOF_LAUNCH_FARM_INTERFACE, 'balanceOf', accountArg)
  const earnedAmounts = useMultipleContractSingleData(rewardsAddresses, BYOF_LAUNCH_FARM_INTERFACE, 'tokensEarned', accountArg)
  const claimedSplits = useMultipleContractSingleData(rewardsAddresses, BYOF_LAUNCH_FARM_INTERFACE, 'claimedSplits', accountArg)
  const totalSupplies = useMultipleContractSingleData(rewardsAddresses, BYOF_LAUNCH_FARM_INTERFACE, 'totalSupply')
  const hasClaimed = useMultipleContractSingleData(rewardsAddresses, BYOF_LAUNCH_FARM_INTERFACE, 'hasClaimed', accountArg)
  const totalEarnedReward = useMultipleContractSingleData(rewardsAddresses, BYOF_LAUNCH_FARM_INTERFACE, 'totalEarnedRewards', accountArg)
  const totalVestedAmount = useMultipleContractSingleData(rewardsAddresses, BYOF_LAUNCH_FARM_INTERFACE, 'totalVestedRewards', accountArg)

  // tokens per second, constants
  const rewardRates = useMultipleContractSingleData(
    rewardsAddresses,
    BYOF_LAUNCH_FARM_INTERFACE,
    'tokensRewardRate',
    undefined,
    NEVER_RELOAD
  )
...
export function useInactiveStakingCustomLaunchFarmInfo(pairToFilterBy?: string): StakingInfo[] {
  const { chainId, account } = useActiveWeb3React()
  ...
  const info = useMemo(
    () =>
      chainId
        ? inactiveLaunchFarms?.filter(stakingRewardInfo =>
          pairToFilterBy === undefined
            ? true
            : pairToFilterBy === null
              ? false
        ...
        : [],
    [chainId, pairToFilterBy, inactiveLaunchFarms]
  )
...
export function useTotalUniEarned(): TokenAmount | undefined {
  ...
  const uni = chainId ? UNI[chainId] : undefined
  const activeStakingInfos = useStakingCustomLaunchFarmInfo()
  const inactiveStakingInfos = useInactiveStakingCustomLaunchFarmInfo();
