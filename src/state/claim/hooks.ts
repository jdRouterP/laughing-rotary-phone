...
function fetchClaim(account: string, chainId: ChainId): Promise<any | null> {
  ...
  return (CLAIM_PROMISES[key] =
    CLAIM_PROMISES[key] ??
    fetch('https://raw.githubusercontent.com/ss-sonic/merkel-path/master/mrkl_claim.json')
      .then(res => {
        return (res.ok ? res.json() : console.log(`No claim for account ${formatted} on chain ID ${chainId}`))
      })
